...
---
`Enumeration` is identifying all of the ways we could attack a target we must find. Enumeration is collecting as much information as possible. The more information we have, the easier it will be for us to find vectors of attack.
...
```
## Host Discovery
To conduct an internal penetration test for the entire network of a company, get an overview of which systems are online that we can work with. 

To actively discover such systems on the network, we can use various `Nmap` host discovery options. The most effective host discovery method is to use **ICMP echo requests**.
...
[Discovery Strategy](https://nmap.org/book/host-discovery-strategies.html)

> [!TIPS]
> You can determine host OS by `ttl` from ping. The default initial TTL value for **Linux/Unix** is **64**, and TTL value for **Windows** is **128**.
...
```shell-session
CaffeineMocha@htb[/htb]$ sudo nmap 10.129.2.28 -p 21,22,25 -sS -Pn -n --disable-arp-ping --packet-trace
...
```shell-session
CaffeineMocha@htb[/htb]$ sudo nmap 10.129.2.28 -p 21,22,25 -sA -Pn -n --disable-arp-ping --packet-trace
...
```shell-session
CaffeineMocha@htb[/htb]$ sudo nmap 10.129.2.28 -p 80 -sS -Pn -n --disable-arp-ping --packet-trace -D RND:5
...
```shell-session
CaffeineMocha@htb[/htb]$ sudo nmap 10.129.2.28 -n -Pn -p445 -O
...
```shell-session
CaffeineMocha@htb[/htb]$ sudo nmap 10.129.2.28 -n -Pn -p 445 -O -S 10.129.2.200 -e tun0
...
```shell-session
CaffeineMocha@htb[/htb]$ sudo nmap 10.129.2.28 -p50000 -sS -Pn -n --disable-arp-ping --packet-trace
...
```shell-session
CaffeineMocha@htb[/htb]$ sudo nmap 10.129.2.28 -p50000 -sS -Pn -n --disable-arp-ping --packet-trace --source-port 53
...
```shell-session
CaffeineMocha@htb[/htb]$ ncat -nv --source-port 53 10.129.2.28 50000
